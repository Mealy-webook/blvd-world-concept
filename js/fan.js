...
(function () {
  const deck = document.getElementById("ride-fan");
  if (!deck) return;
  ...
  /* ── tweening: just the easings the fan needs ── */
  const easings = {
  ...
  function visibleMap(c) {
    const map = new Map();
    if (!paginated) { data.forEach((_, i) => map.set(i, i)); return map; }
  ...
  const dotsBox = document.getElementById("ride-dots");
  if (dotsBox && paginated) {
    dotsBox.innerHTML = data.map((_, i) => `<button type="button" aria-label="Ride ${i + 1}"></button>`).join("");
  }
  ...
  function goTo(i) {
    ...
    cycle(forward <= total / 2 ? "right" : "left");
    // step the rest of the way once each hop settles
    if (i !== center) {
  ...
  /* ── wiring ── */
  const prev = document.getElementById("ride-prev");
  const next = document.getElementById("ride-next");
  ...
  deck.addEventListener("mouseover", (e) => {
    ...
    const slot = visibleMap(center).get(cards.indexOf(card));
  ...
  // changes — including the first time the deck is given a real size. A change
  // mid-flight is deferred to settle(), otherwise the deck would keep the
  // multipliers it was dealt with.
  let lastMetrics = "";
  function relayout() {
