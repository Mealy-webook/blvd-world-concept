// ── the fanned deck ───────────────────────────────────────────────
// Ported from the React `card-fan-carousel` component, and restored here from
// commit aaad543 where the zones last used it. The geometry is carried over
// verbatim — the slot table, the responsive multipliers, the hover push that
// makes the neighbours part around the card under the cursor — with GSAP
// replaced by the small tween loop below, so the page keeps its no-bundler,
// no-CDN-on-the-critical-path setup.
//
// It drives the zones in the globe version. The ring in js/coverflow.js drives
// them everywhere else; both read the same order from WBK_ZONES, and both answer
// a click the same way: a card off the front comes forward, and the front card
// opens that zone on the map.
(function () {
  const MAX_VISIBLE = 7;
  const HALF = 3;
  ...
  function makeFan({ deckId, prevId, nextId, dotsId, items, card }) {
    ...
    deck.addEventListener("mouseover", (e) => {
      ...
      // what the cursor should call this card: the front one leads to the map,
      // the rest only bring themselves forward. js/cursor.js reads the attribute.
      deck.dataset.cursorWord = i === center ? "Explore zone" : "Select";
    ...
    deck.addEventListener("mouseleave", () => {
      delete deck.dataset.cursorWord;
      if (animating) return;
  ...
  /* ── the zones ──
     Eight posters, which is more than the seven slots the fan shows at once, so
     it pages: the arrows and the dots are live and the deck rolls a card in from
     whichever side you are heading towards. The card is a link — the front one
     opens that zone on the map, and clicking any other brings it forward first,
     which is the same contract the coverflow ring uses. */
  if (window.WBK_ZONES) {
    const { items, isNew } = WBK_ZONES;
    makeFan({
      deckId: "zone-deck", prevId: "zone-prev", nextId: "zone-next", dotsId: "zone-dots",
      items,
      card: (z) => `
        <a class="fan-card is-zone" href="#/map?zone=${encodeURIComponent(z.name)}"
